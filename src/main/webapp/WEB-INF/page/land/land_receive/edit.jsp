...
<div class="pageContent">
	<form method="post" action="${pageContext.request.contextPath}${requestScope.record ==null ?'/land/land_receive/insert' :'/land/land_receive/update' }" class="pageForm required-validate"
		onsubmit="return validateCallback(this, dialogAjaxDone);">
		<input type="hidden" name="olid" value="${requestScope.olid}">
		<div class="pageFormContent" layoutH="56">
			...
			<p>
			<label>土地位置：</label> 
			<select class="conbox" name="lid" >
				<option value="">请选择</option>
					<c:forEach items="${requestScope.land}" var="item">
						<option value="${item.id}" ${item.id==requestScope.record.lid?'selected':''}>${item.address}</option>
					</c:forEach>
			</select>
			</p>
			<p>
