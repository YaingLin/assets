...
<div class="pageContent">
	<form method="post" action="${pageContext.request.contextPath}${requestScope.record ==null ?'/house/house/insert' :'/house/house/update' }" class="pageForm required-validate"
		...
		<div class="pageFormContent" layoutH="56">
			...
			<p>
				<label>房屋性质：</label> 
				<input name="property" class="required" maxlength="100" type="text" value="${requestScope.record.property }" placeholder="房屋性质" />
			</p>
			<p>
			<label>屋主：</label> 
			<select class="conbox" name="owner" >
				<option value="">请选择</option>
					<c:forEach items="${requestScope.user}" var="item">
						<option value="${item.id}" ${item.id==requestScope.record.owner?'selected':''}>${item.name}</option>
					</c:forEach>
			</select>
			</p>
			<p>
				<label>房屋位置：</label> 
				<input name="address" class="required" maxlength="100" type="text" value="${requestScope.record.address }" placeholder="房屋位置" />
			</p>
			<p>
				<label>房屋面积：</label> 
				<input name="area" class="required"  maxlength="10" type="text" value="${requestScope.record.area }" placeholder="房屋面积" />
			</p>
			<p>
				<label>使用期限：</label> 
